...
import type { AssetCategory } from '@/lib/hooks';
                ...
                <TableBody>
                  {(categories ?? []).map(cat => (
                    <TableRow key={cat.id}>
                      ...
                      <TableCell className="hidden sm:table-cell text-muted-foreground">{Number(cat.residual_value_percent ?? 0)}%</TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" className="h-10 w-10 sm:h-8 sm:w-8 text-muted-foreground hover:text-foreground" onClick={() => openEdit(cat)}>
